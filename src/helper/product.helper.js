...
exports.getProductItemsWithFilters = async (payload) => {
  try {
    ...
    const result = await ProductItem.aggregate(aggregateQuery).skip(Number(payload.skip)).limit(Number(payload.limit)).exec();
    return result;
  } catch (error) {
    throw error;
  }
...
exports.getProductItemsByShopAndCategory = async (params) => {
  ...
  try {
    ...
    const productItems = await ProductItem.aggregate(query).skip(Number(skip)).limit(Number(limit)).exec();
    return productItems;
  } catch (error) {
    throw error
  }
...
exports.getProductItemRatingsByProductItemId = async (productItemId) => {
  try {
    ...
    const productRatings = await ProductRating.aggregate(query).limit(1).exec();
